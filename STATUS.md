...
Phases 0–9 merged into `main`. Phase 9 (First Workers: Research + Memory) landed via merge commit `b74e597`, followed by a regression fix for the missing-key path (`879a48f`), five issues surfaced by the 2026-05-13 code reviews (`37499e4`), repo hygiene around `.specstory/` and legacy `.claude/handoffs/` (`335135b`, `cd59277`), and README/docs updates (`b710591`). Real worker agents are now in the system: `MasterAgent.execute` is a thin delegate to `WorkflowRunner.execute`, which dispatches the agents listed in `workflow.agents` in sequential order, threads each agent's output forward via `AgentInput.prior_findings`, and writes one `agent_runs` row per dispatch. `config/workflows.yaml` declares the agents + mode per workflow name; `research_brief` runs `["research", "persona:architect"]` and other workflows stay single-persona. The Research Agent retrieves cross-conversation messages by keyword overlap and greps recent daily-note files; the Memory Agent owns the assistant-message write and the `after_send` vault projection (single-writer rule; soft enforcement in production, strict-mode pytest fixture verifies). `workflow_runs` rows are INSERTed with `outcome='in_progress'` before execute and UPDATEd after, so the runner can FK-link `agent_runs` immediately. `/agents` lists the registered workers. Next up: Phase 10 (Evaluator + Critic + Persona Agents) on `phase-10-evaluator-critic`. v0.1 scope is multi-agent + self-improving + CLI; see [UBONGO_BUILD.md](UBONGO_BUILD.md).
