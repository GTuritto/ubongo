...
**Profiler** (`ubongo.profiling`, candidate 10):
The local profiler, two stdlib-only halves behind `/profile`. The **stats half** aggregates the `workflow_runs` / `agent_runs` rows the runner already persists — on demand, read-only (the Memory Agent's single-writer rule untouched) — into a summary plus per-agent / per-model / per-mode breakdowns (avg and nearest-rank p95 latency, tokens, failure and retry rates), optionally windowed to the last N workflow runs. The **CPU half** is armed explicitly (`/profile cpu on`, or `ubongo send --profile`) and wraps the turn's `master.handle` in `cProfile`, dumping `data/profiles/turn-<ts>.prof` and emitting a top-25 cumulative summary; profiling is best-effort and can never break a turn. Zero overhead when off.
