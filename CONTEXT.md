...
**Signal channel** (`ubongo.signal`, v0.7):
The seventh channel and the second cloud-relayed one — Signal, the privacy-respecting platform preferred over Telegram. Same shape as Telegram (additive over `channel.run_turn`, no bypass): `signal/service.py` is the transport-free core (auth via `signal.allowed_numbers`, empty = deny all; the `/approve|/decline|/pending|/grants` router; approve-later over Signal via `master.resume_approval`); `signal/client.py` is the only module speaking to the transport. The one structural difference from Telegram: the transport is a locally-run **signal-cli daemon** reached over a JSON-RPC UNIX socket — so no token in config (the credential is signal-cli's own keystore) and no pip dependency (pure stdlib). Cloud-relayed like Telegram; the signal-cli daemon runs inside the egress envelope with Signal's hosts allowlisted (ADR-0024/0017).
_Avoid_: bot (it's a channel), SDK (there is none — a sidecar process), Signal API (there is none — signal-cli wraps libsignal).
