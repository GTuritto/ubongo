...
1. **The v0.6 fork is now: roster (Phase 01) is the headline, and it's where Ubongo beats the reference UIs.** Because the agent lineup is planned up front (`WorkflowPlan`), the console can show "working now / queued / retiring" as fact, not a guess — unlike a dynamic-spawn coding agent. This is the highest-leverage, lowest-risk next build, and the transport (Phase 00) already exists.
