...
| 1.7 | Unknown slash command | In REPL: `/foo` | `Unknown command: /foo. Try /architect, /operator, /casual, /auto, /skill <name>, /skills, /summary, /queue, /decisions, /agents, /trace, /reload, /exit.` Loop continues. (Help line grows as later phases add commands.) |
...
| 9.3 | `/agents` | REPL `/agents` | Header `Registered agents:` and rows for `architect`, `casual`, `critic`, `evaluator`, `memory`, `operator`, `research` with role + model. (Bare persona names + Evaluator/Critic land in Phase 10; in earlier Phase-9 form the names were `persona:<x>` and the table excluded evaluator/critic.) |
...
| 10.4 | `/agents` updated table | REPL `/agents` | Header `Registered agents:` with seven rows: `architect`, `casual`, `critic`, `evaluator`, `memory`, `operator`, `research`. Each has role + model (memory shows `—` for model). |
