...
## v0.7 — The Signal channel (phases 00 + 01)

The seventh channel and the privacy-respecting one preferred over Telegram ([Plans/signal-channel.md](../../Plans/signal-channel.md), [ADR-0024](../../docs/adr/0024-signal-channel.md)): drive Ubongo from Signal, with approve-later reaching you over Signal. Additive over `channel.run_turn` (no orchestration change). `signal/service.py` is the transport-free core; `signal/client.py` is the only module that speaks JSON-RPC to the signal-cli daemon. Auth returns via `signal.allowed_numbers` (empty = deny all). The one structural difference from Telegram: the transport is a local signal-cli daemon, not a pip SDK — no token in config, no pip dependency. Most steps are headless (service + parse/frame unit-tested); the live daemon round-trip needs signal-cli (deferred — see docs/signal-setup.md, incl. the Mock-Signal-Server option).

| # | Step | Command | Expected |
| --- | --- | --- | --- |
| AA.1 | No daemon is friendly | `python -m ubongo signal` (unset/empty `signal.socket`) | rc 1; "Error: signal.socket is not set…" / "socket not found…" + a pointer to docs/signal-setup.md; no traceback. |
| AA.2 | Unauthorized sender refused, no turn | `uv run pytest tests/test_signal_service.py -k unauthorized` | `handle_message` returns "Not authorized." and `channel.run_turn` is never called (empty `allowed_numbers` denies all). |
| AA.3 | Authorized normal turn, no bypass | `uv run pytest tests/test_signal_service.py -k normal_turn` | the turn routes through `channel.run_turn(..., auto_mode=True)` → `master.handle`; reply is the composed text. |
| AA.4 | Gated turn surfaces the decision_id | `uv run pytest tests/test_signal_service.py -k gated` | the reply carries the gated text plus `/approve <id>` and `/decline <id>` (over Signal). |
| AA.5 | Command router (approve-later) | `uv run pytest tests/test_signal_service.py -k "pending or approve or decline or grants"` | `/pending`, `/approve`, `/decline`, `/grants` resolve through the Phase-03/05 seams (`master.resume_approval`, `grant_state`) — no re-implemented resume. |
| AA.6 | Transport parse/frame (socket-free) | `uv run pytest tests/test_signal_client.py` | a `receive` notification parses to (source, text); receipts/self/empty are ignored; a `send` frames valid JSON-RPC; a send error is swallowed. |
| AA.7 | Stdlib-only, no core dep | (no `[signal]` extra exists) `uv run python -c "from ubongo.signal import service, client"` | imports fine; the client uses only stdlib `socket`/`json`; core imports without touching the transport. |
| AA.8 | ctl + service control (Pi) | `./ubongo-ctl.sh start signal`, `status signal`, `stop signal` | start backgrounds the channel (pidfile `data/ubongo-signal.pid`); status exits 0; stop terminates. (Needs a running signal-cli daemon.) |
| AA.9 | Egress enumerable | `grep signal.org deploy/envelope/egress.hosts` | `chat.signal.org` + the CDNs are allowlisted; the signal-cli daemon runs inside the envelope (ADR-0024/0017). |
| AA.10 | Live round-trip (deferred) | stand up signal-cli (real number or Mock-Signal-Server) per docs/signal-setup.md; `./start-ubongo-signal.sh`; message the dedicated number | an allowed number gets a real composed reply; an unlisted number gets "Not authorized."; a destructive prompt gates and `/approve <id>` delivers over Signal. |
| AA.11 | Pytest passes | `uv run pytest` | full suite green (`test_signal_service.py` + `test_signal_client.py` included). |
