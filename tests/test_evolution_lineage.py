...
def test_lineage_for_target_filters_by_generation(db) -> None:
    evolution_state.append_lineage_variant(target="t", parent_id=None, generation=1,
                                 variant_text="a", variant_metadata=None)
    evolution_state.append_lineage_variant(target="t", parent_id=None, generation=2,
                                 variant_text="b", variant_metadata=None)
    assert len(evolution_state.lineage_for_target("t", generation=1)) == 1
    assert len(evolution_state.lineage_for_target("t")) == 2


def test_max_lineage_generation(db) -> None:
    assert evolution_state.max_lineage_generation("t") == 0
    evolution_state.append_lineage_variant(target="t", parent_id=None, generation=1,
                                 variant_text="a", variant_metadata=None)
    evolution_state.append_lineage_variant(target="t", parent_id=None, generation=3,
                                 variant_text="b", variant_metadata=None)
    assert evolution_state.max_lineage_generation("t") == 3


def test_active_lineage_id_none_then_set(db) -> None:
    assert evolution_state.active_lineage_id("persona:architect") is None
    lid = evolution_state.append_lineage_variant(target="persona:architect", parent_id=None,
    ...
    assert evolution_state.active_lineage_id("persona:architect") == lid
...
def test_record_variants_increments_generation(db) -> None:
    ...
    lineage.record_variants("persona:architect", _variants(3))
    gens = {r["generation"] for r in evolution_state.lineage_for_target("persona:architect")}
    assert gens == {1, 2}
...
def test_record_variants_parent_points_to_active(db) -> None:
    ...
    first = evolution_state.append_lineage_variant(target="persona:architect", parent_id=None,
    ...
    lineage.record_variants("persona:architect", _variants(2))
    new_rows = [r for r in evolution_state.lineage_for_target("persona:architect") if r["id"] != first]
...
def test_record_empty_variants_noop(db) -> None:
    ...
    assert evolution_state.lineage_for_target("persona:architect") == []
...
def test_record_variants_honors_per_variant_parent_id(db) -> None:
    ...
    parent = evolution_state.append_lineage_variant(
    ...
    child = [r for r in evolution_state.lineage_for_target("persona:architect") if r["id"] == ids[0]][0]
