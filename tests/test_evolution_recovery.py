...
def test_unevaluated_generation_is_reevaluated_not_regenerated(db, monkeypatch) -> None:
    ...
    assert store.evaluations_for_target("persona:architect", generation=1) == []
    ...
    assert store.max_lineage_generation("persona:architect") == 1
    assert len(store.evaluations_for_target("persona:architect", generation=1)) == 3


def test_interrupted_runs_are_listed(db) -> None:
    rid = store.start_evolution_run(target="persona:casual", generation=1)
    assert [r["id"] for r in store.interrupted_evolution_runs()] == [rid]
    store.finish_evolution_run(rid, calls_spent=5, outcome="completed")
    assert store.interrupted_evolution_runs() == []
