...
def _lineage(target: str, gen: int, text: str, strategy: str) -> int:
    return evolution_state.append_lineage_variant(
...
def test_append_and_read_evaluation(db) -> None:
    ...
    eid = evolution_state.append_evaluation(
...
def test_evaluations_ordered_fitness_desc_then_lineage_asc(db) -> None:
    ...
    evolution_state.append_evaluation(lineage_id=l1, sample_set="s", success_rate=0.5,
                            cost=1, latency_ms=1, hallucination_rate=0,
                            user_correction_rate=0, fitness=0.50)
    evolution_state.append_evaluation(lineage_id=l2, sample_set="s", success_rate=0.9,
                            cost=1, latency_ms=1, hallucination_rate=0,
                            user_correction_rate=0, fitness=0.90)
    # l3 ties l2 on fitness -> lower lineage_id (l2) ranks first.
    evolution_state.append_evaluation(lineage_id=l3, sample_set="s", success_rate=0.9,
                            cost=1, latency_ms=1, hallucination_rate=0,
                            user_correction_rate=0, fitness=0.90)
    rows = evolution_state.evaluations_for_target("persona:casual")
...
def test_evaluations_generation_filter(db) -> None:
    ...
    evolution_state.append_evaluation(lineage_id=l2, sample_set="s", success_rate=0.6,
                            cost=1, latency_ms=1, hallucination_rate=0,
                            user_correction_rate=0, fitness=0.6)
    assert len(evolution_state.evaluations_for_target("persona:operator", generation=1)) == 1
    assert len(evolution_state.evaluations_for_target("persona:operator")) == 2


def test_latest_evaluation_for_lineage(db) -> None:
    lid = _lineage("persona:architect", 1, "x", "expand")
    assert evolution_state.latest_evaluation_for_lineage(lid) is None
    evolution_state.append_evaluation(lineage_id=lid, sample_set="s", success_rate=0.5,
                            cost=1, latency_ms=1, hallucination_rate=0,
                            user_correction_rate=0, fitness=0.5)
    evolution_state.append_evaluation(lineage_id=lid, sample_set="s", success_rate=0.7,
                            cost=1, latency_ms=1, hallucination_rate=0,
                            user_correction_rate=0, fitness=0.7)
    latest = evolution_state.latest_evaluation_for_lineage(lid)
