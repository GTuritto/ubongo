...
def _llm_variant(strategy: str, system_prompt: str, user_content: str, metadata: dict) -> Variant | None:
    ...
    if not text:
        ...
        return None
    return Variant(strategy=strategy, text=text, metadata=metadata)


def _make_variant(strategy: str, target: str, base: str, occurrence: int) -> Variant | None:
...
def generate(target: str, n: int) -> list[Variant]:
    ...
    # Cap total attempts so an all-skipping target (no peer + LLM down) can't
    # spin: at most 2 full passes over the strategy list beyond n.
    ...
    while len(variants) < n and attempt < max_attempts:
        ...
        i += 1
        attempt += 1
        variant = _make_variant(strategy, target, base, occurrences[strategy])
        if variant is None:
            continue
