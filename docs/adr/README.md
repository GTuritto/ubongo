...
**0024** — the Signal channel (v0.7, built + merged): the privacy-respecting messaging channel
Giuseppe prefers, an additive adapter over the one turn seam in the Telegram pattern (ADR-0020).
The one structural difference is that the transport is a locally-run signal-cli sidecar spoken to
over JSON-RPC, not a pip SDK; a dedicated number, fail-closed `allowed_numbers`, no secret in
config. Phase 00 shipped the sidecar + turn round-trip; Phase 01 shipped the command router +
approve-later over Signal, the ops surfaces, and the egress allowlist. It does *not* supersede
0020 (Telegram stays a peer channel).
